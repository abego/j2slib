...
$_V(c$,"getBounds",
function(){
return this.getPrimaryMonitor().getClientArea();
});
...
monitor.clientY=$wt.widgets.QuickLaunch.BAR_HEIGHT;
monitor.clientHeight-=36+$wt.widgets.QuickLaunch.BAR_HEIGHT;
}else if("top".equals(orientationStr)){
monitor.clientY=36;
monitor.clientHeight-=36;
...
scrolling=1.0*d$.body.scrollTop/d$.body.scrollHeight;
}else{
scrolling=1.0*d$.body.parentNode.scrollTop/d$.body.parentNode.scrollHeight;
...
var clientArea=this.getPrimaryMonitor().getClientArea();
...
if(scrolling!=0){
panel.scrollTop=Math.round(scrolling*panel.scrollHeight);
}}if(this.taskBar.orientation!=1024){
...
if(this.timerIds==null)this.timerIds=$_AI(4,0);
var index=0;
...
var newTimerIds=$_AI(this.timerIds.length+4,0);
...
zidx=Integer.parseInt(idx);
}if(zidx>lastZIndex&&(ss[i].style&(229376))==0){
lastZIndex=zidx;
lastShell=ss[i];
}}}
}
...
var ss=disps[k].getShells();
for(var i=0;i<ss.length;i++){
if(!ss[i].isDisposed()&&ss[i].handle.style.display!=="none"){
O$.removeCSSClass(ss[i].handle,"shell-active");
O$.addCSSClass(ss[i].handle,"shell-inactive");
}}
}
});
...
disp.updateLayout();
}if(O$.isMobile&&d$.body.scrollLeft>0){
var plainWidth=Math.max(d$.body.scrollWidth,d$.body.parentNode.scrollWidth);
var ratio=1.0;
var displayWidth=O$.getFixedBodyClientWidth();
var screenWidth=displayWidth;
{
ratio=window.outerWidth/plainWidth;
screenWidth=(window.orientation==90||window.orientation==-90)?window.screen.height:window.screen.width;
}if(d$.body.scrollLeft*ratio+screenWidth>displayWidth*ratio){
w$.scrollTo($_dI(Math.floor(displayWidth-screenWidth/ratio)),d$.body.scrollTop);
}}});
...
}if($wt.widgets.Display.hShellScrolling!=null){
Clazz.removeEvent(document,"touchend",$wt.widgets.Display.hShellScrolling);
($t$=$wt.widgets.Display.hShellScrolling=null,$wt.widgets.Display.prototype.hShellScrolling=$wt.widgets.Display.hShellScrolling,$t$);
}O$.dispose();
});
...
Clazz.addEvent(document,O$.isMobile?"touchstart":"click",$wt.widgets.Display.hShellZOrdering);
if(O$.isMobile){
($t$=$wt.widgets.Display.hShellScrolling=$_Q((($_D("$wt.widgets.Display$5")?0:org.eclipse.swt.widgets.Display.$Display$5$()),$_N($wt.widgets.Display$5,this,null))),$wt.widgets.Display.prototype.hShellScrolling=$wt.widgets.Display.hShellScrolling,$t$);
Clazz.addEvent(document,"touchend",$wt.widgets.Display.hShellScrolling);
}});
...
var x=e.clientX;
var y=e.clientY;
if((this.b$["$wt.widgets.Display"].taskBar.orientation==16384?x>264:x<this.b$["$wt.widgets.Display"].taskBar.clientWidth-264)&&y>=100&&y<=bottom){
return;
...
if(this.b$["$wt.widgets.Display"].taskBar.barEl!=null&&(this.b$["$wt.widgets.Display"].taskBar.orientation==16384?x<264:x>this.b$["$wt.widgets.Display"].taskBar.clientWidth-264)){
...
c$.$Display$5$=function(){
$_H();
c$=$_W($wt.widgets,"Display$5",$wt.internal.RunnableCompatibility);
$_M(c$,"run",
function(){
...
w$.setTimeout($_Q((($_D("$wt.widgets.Display$5$1")?0:org.eclipse.swt.widgets.Display.$Display$5$1$()),$_N($wt.widgets.Display$5$1,this,null))),100);
}});
c$=$_P();
};
c$.$Display$5$1$=function(){
$_H();
c$=$_W($wt.widgets,"Display$5$1",null,Runnable);
$_M(c$,"run",
function(){
if(d$.body.scrollLeft<=0){
return;
