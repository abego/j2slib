...
width+=160;
var scrollY=16;
height+=(thumbHeight*2)+scrollY+($_dI(scrollY/3));
}else{
var scrollX=16;
width+=(thumbWidth*2)+scrollX+($_dI(scrollX/3));
height+=160;
...
}var pages=$_dI((this.maximum-this.minimum)/this.pageIncrement);
...
line.style.left=Math.floor($_dI(range*i/ pages) + $_dI (thumbSize /2))+"px";
}else{
line.style.top=Math.floor($_dI(range*i/ pages) + $_dI (thumbSize /2))+"px";
}}
...
this.selection=$_dI(left*this.maximum/(this.getSize().x-12));
}else{
var top=thumbStyle.top.length>0?Integer.parseInt(thumbStyle.top):0;
this.selection=this.maximum-$_dI(top*this.maximum/(this.getSize().y-12));
...
this.thumbHandle.style.left=Math.round($_dI(this.selection*(this.getSize().x-12)/this.maximum))+"px";
}else{
this.thumbHandle.style.top=Math.round($_dI((this.maximum-this.selection)*(this.getSize().y-12)/this.maximum))+"px";
}},"~N");
$_V(c$,"SetWindowPos",
