...
}if(O$.isIE&&!O$.isIE80&&!O$.isIE90){
btnWrapperEl.style.bottom="-0.5em";
}else if(O$.isMobile){
btnWrapperEl.style.marginTop="3px";
...
this.btnHandle.style.textAlign="right";
}if(O$.isChrome){
...
$_M(c$,"forceFocus",
function(){
var focused=$_U(this,$wt.widgets.Button,"forceFocus",[]);
O$.SetFocus(this.btnHandle);
return focused;
});
$_M(c$,"getAlignment",
function(){
...
}this.btnIcon.style.width=(this.image.width+1)+"px";
}else{
this.btnIcon.style.position="absolute";
this.btnIcon.style.paddingLeft="0px";
...
this.btnText.appendChild(d$.createTextNode(string.substring(0,idx)+string.substring(idx+1)));
}else{
...
var h=0;
if(this.textSizeCached&&!O$.isIE80&&!O$.isIE90){
this.btnText.style.display="block";
if(this.textHeightCached<13){
this.btnText.style.paddingTop=($_dI((13-this.textHeightCached)/2))+"px";
...
this.btnText.parentNode.style.top=($_dI((cy-h)/2))+"px";
}}var el=hWnd;
el.style.left=X+"px";
el.style.top=Y+"px";
var w=cx>0?cx:0;
