...
var lastTopShell=null;
if(parentShell&&childShells){
lastTopShell=$wt.widgets.Display.getTopShell();
...
}O$.removeCSSClass(this.handle,"shell-inactive");
O$.addCSSClass(this.handle,"shell-active");
...
function(handle){
var handles=["shell-left-top","shell-right-top","shell-center-top","shell-left-middle","shell-right-middle","shell-left-bottom","shell-right-bottom","shell-center-bottom"];
...
for(var i=0;i<handles.length;i++){
var cssName=handles[i];
if(cssName!=null){
var el=$wt.widgets.Decorations.createCSSDiv(handle,cssName);
if(O$.isSafari&&O$.isMobile){
if(cssName.indexOf("left-top")!=-1){
el.style.left="-15px";
el.style.top="-15px";
el.style.width="15px";
el.style.height="15px";
}else if(cssName.indexOf("center-top")!=-1){
el.style.top="-15px";
el.style.height="15px";
}else if(cssName.indexOf("right-top")!=-1){
el.style.right="-15px";
el.style.top="-15px";
el.style.width="15px";
el.style.height="15px";
}else if(cssName.indexOf("left-middle")!=-1){
el.style.left="-15px";
el.style.width="15px";
}else if(cssName.indexOf("right-middle")!=-1){
el.style.right="-15px";
el.style.width="15px";
}else if(cssName.indexOf("left-bottom")!=-1){
el.style.left="-15px";
el.style.bottom="-15px";
el.style.width="15px";
el.style.height="15px";
}else if(cssName.indexOf("center-bottom")!=-1){
el.style.bottom="-15px";
el.style.height="15px";
}else if(cssName.indexOf("right-bottom")!=-1){
el.style.right="-15px";
el.style.bottom="-15px";
el.style.width="15px";
el.style.height="15px";
...
function(handle){
$wt.widgets.Decorations.appendNarrowShadowHandles(handle,true,true,true,true);
},"Element");
c$.appendNarrowShadowHandles=$_M(c$,"appendNarrowShadowHandles",
function(handle,top,right,bottom,left){
var handles=[left&&top?"shadow-narrow-left-top":null,right&&top?"shadow-narrow-right-top":null,top?"shadow-narrow-center-top":null,left?"shadow-narrow-left-middle":null,right?"shadow-narrow-right-middle":null,"shadow-narrow-center-middle",left&&bottom?"shadow-narrow-left-bottom":null,right&&bottom?"shadow-narrow-right-bottom":null,bottom?"shadow-narrow-center-bottom":null];
for(var i=0;i<handles.length;i++){
var cssName=handles[i];
if(cssName!=null){
var el=$wt.widgets.Decorations.createCSSDiv(handle,cssName);
...
$wt.widgets.Decorations.createCSSDiv(this.handle,"shell-center-middle");
...
}if(supportShadow&&(this.style&8)==0){
...
this.modalHandle.style.zIndex=this.handle.style.zIndex-1;
if(O$.isOpera){
this.modalHandle.style.backgroundColor="transparent";
...
y-=$_dI((20-this.image.packedItemHeight)/2);
...
this.setBounds(this.computeTrim(x,y,width,height-titleHeight));
}else{
var trim=this.computeTrim(x,y-titleHeight,width,height);
...
}if(this.display.topBar!=null){
this.display.topBar.returnTopMaximized(null);
this.display.topBar.handleApproaching();
this.display.topBar.updateLayout();
this.display.topBar.updateLastModified();
}return;
}}if(this.display.taskBar!=null){
this.handle.style.display=minimized?"none":"";
...
this.contentHandle.scrollTop=0;
...
function(width){
if(this.shellTitle.parentNode!==this.titleBar&&this.maximized){
return;
}var ww=18;
var w=ww;
if(this.shellClose!=null){
...
this.b$["$wt.widgets.Decorations"].display.topBar.returnTopMaximized(shell);
if(this.b$["$wt.widgets.Decorations"].display.topBar.isVisible()){
this.b$["$wt.widgets.Decorations"].display.topBar.updateLayout();
