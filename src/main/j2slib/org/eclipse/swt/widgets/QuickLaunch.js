...
if(this.display.taskBar!=null&&this.display.taskBar.orientation==1024){
$wt.widgets.Decorations.appendNarrowShadowHandles(this.handle,false,false,true,false);
}else{
$wt.widgets.Decorations.appendShadowHandles(this.handle,true,true,false,true);
}}else{
if(this.display.taskBar!=null&&this.display.taskBar.orientation==1024){
this.handle.style.borderBottom="1px solid black";
...
var zIndex=this.getNextLayerLevel(false);
this.handle.style.zIndex=zIndex;
this.handle.style.left="0px";
this.handle.style.top="0px";
this.handle.style.height=($wt.widgets.QuickLaunch.BAR_HEIGHT-1)+"px";
this.handle.style.width=this.display.taskBar.clientWidth+"px";
{
...
this.handle.style.display="none";
($t$=$wt.widgets.QuickLaunch.BAR_HEIGHT=0,$wt.widgets.QuickLaunch.prototype.BAR_HEIGHT=$wt.widgets.QuickLaunch.BAR_HEIGHT,$t$);
}});
...
if(zIdx==-1){
zIndex=this.getNextLayerLevel(true);
...
$_V(c$,"updateLayout",
function(){
if(this.display!=null&&this.display.taskBar!=null&&(this.display.taskBar.orientation==1024)){
this.handle.style.width=this.display.taskBar.clientWidth+"px";
}if(this.shortcutCount<=0){
return;
...
barOffset=O$.getFixedBodyClientWidth()-barWidth;
}else{
barOffset=$_dI((O$.getFixedBodyClientWidth()-barWidth)/2);
}if(this.handle!=null){
if(this.display.taskBar!=null&&this.display.taskBar.orientation==1024){
this.handle.style.left="0px";
this.handle.style.top="0px";
...
this.initialize();
}else if(this.handle.style.display==="none"){
if($wt.widgets.QuickLaunch.BAR_HEIGHT!=0){
...
function(item){
for(var i=0;i<this.shortcutCount;i++){
if(this.shortcutItems[i]===item){
Clazz.removeEvent(item,"mouseover",this.hLaunchMouseEnter);
d$.body.removeChild(item);
this.shortcutItems[i]=null;
for(var j=i;j<this.shortcutCount-1;j++){
this.shortcutItems[j]=this.shortcutItems[j+1];
}
this.shortcutCount--;
this.updateLayout();
return;
...
$_M(c$,"getNextLayerLevel",
($fz=function(increasing){
...
}var zIndex=this.b$["$wt.widgets.QuickLaunch"].getNextLayerLevel(false);
