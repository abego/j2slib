...
$_V(c$,"updateLayout",
function(){
this.clientWidth=O$.getFixedBodyClientWidth();
this.clientHeight=O$.getFixedBodyClientHeight();
this.updateItems();
});
...
si.className="shell-item"+((this.orientation==131072)?" shell-item-right":"");
if(tag==="A"){
si.href="#";
}si.title=text;
...
if(w>120){
w=120;
}var showShortcut=false;
if(this.orientation==1024||this.orientation==128){
...
this.updateItems();
var existed=this.items.length>0||(this.display!=null&&this.display.tray!=null&&this.display.tray.itemCount>0);
{
...
var offset=O$.getFixedBodyOffsetTop()+Math.round($_dI((height-barHeight)/2));
var y1=offset-72;
...
if(length==0){
if(this.orientation==1024){
this.barEl.style.height="34px";
this.barEl.style.width=this.clientWidth+"px";
this.barEl.style.top=(this.clientHeight-36)+"px";
}else if(this.orientation==128){
this.barEl.style.height="34px";
this.barEl.style.width=this.clientWidth+"px";
...
var offset=0;
var height=O$.getFixedBodyClientHeight();
offset=O$.getFixedBodyOffsetTop()+Math.round($_dI((height-36)/2));
...
var offset=O$.getFixedBodyOffsetTop()+Math.round($_dI((height-(length*hh+36))/2));
var topShell=$wt.widgets.Display.getTopShell();
var hideLogo=false;
{
hideLogo=window["swt.logo"]==false;
}var offsetX=(hideLogo||this.display.trayCorner==null||this.display.trayCorner.logoOrientation==131072)?8:72;
if(this.display.trayCorner!=null&&this.display.trayCorner.orientation==16384&&this.display.tray!=null){
offsetX+=this.display.tray.itemCount*32+24;
}var items=this.items;
if(this.orientation==1024||this.orientation==128){
items=new Array(this.items.length);
var index=0;
...
if(this.orientation==1024){
item.itemHandle.style.top=(this.clientHeight-32)+"px";
}else{
item.itemHandle.style.top="4px";
}offsetX+=O$.getContainerWidth(item.itemHandle)+(O$.isFirefox?8:4);
}else{
...
item.text=text;
}var tooltips=item.shell.getToolTipText();
...
sb.className="shell-manager-sidebar"+(this.orientation==131072?" shell-manager-sidebar-right":"");
sb.style.lineHeight="16px";
d$.body.appendChild(sb);
this.handle=sb;
var bb=d$.createElement("DIV");
bb.className="shell-manager-bar"+(this.orientation==131072?" shell-manager-bar-right":"");
sb.appendChild(bb);
this.barEl=bb;
this.clientWidth=O$.getFixedBodyClientWidth();
this.clientHeight=O$.getFixedBodyClientHeight();
...
if(this.orientation==131072){
$wt.widgets.Decorations.appendShadowHandles(this.barEl,true,false,true,true);
}else if(this.orientation==1024){
$wt.widgets.Decorations.appendNarrowShadowHandles(this.barEl,true,false,false,false);
}else if(this.orientation==128){
$wt.widgets.Decorations.appendNarrowShadowHandles(this.barEl,false,false,true,false);
}else{
$wt.widgets.Decorations.appendShadowHandles(this.barEl,true,true,true,false);
}}this.configureEvents();
var taskBarOnTop=false;
{
taskBarOnTop=window["swt.task.bar.top"]==true;
}if(taskBarOnTop){
this.handle.style.zIndex=w$.currentTopZIndex+100;
this.isAutoHide=false;
this.setMinimized(false);
}else{
this.barEl.title="Doubleclick to set taskbar auto-hide";
}var existed=(this.display!=null&&this.display.tray!=null&&this.display.tray.itemCount>0);
{
if(window["swt.task.bar.default"]==true){
existed=true;
}
...
var cssName="shell-manager-bar"+(minimized?"-minimized":"");
this.barEl.className=cssName+(this.orientation==131072?" "+cssName+"-right":"");
...
return!ctrlKey&&((this.orientation==131072?x>=this.clientWidth-8:x<=8))&&this.isAround(x,y);
},"~N,~N,~N,~B");
$_M(c$,"isLeaving",
function(now,x,y,ctrlKey){
return ctrlKey||(this.orientation==131072?x<this.clientWidth-200:x>200)||!this.isAround(x,y);
},"~N,~N,~N,~B");
$_V(c$,"releaseWidget",
function(){
if(this.items!=null){
...
this.f$.shell.bringToTop();
if(this.f$.shell.getMaximized()&&this.b$["$wt.widgets.TaskBar"].display.topBar!=null){
this.b$["$wt.widgets.TaskBar"].display.topBar.returnTopMaximized(null);
this.b$["$wt.widgets.TaskBar"].display.topBar.handleApproaching();
this.b$["$wt.widgets.TaskBar"].display.topBar.updateLayout();
this.b$["$wt.widgets.TaskBar"].display.topBar.updateLastModified();
}}}this.b$["$wt.widgets.TaskBar"].updateItems();
this.f$.shell.forceFocus();
this.toReturn(false);
});
c$=$_P();
};
