...
this.allCells[$_dI(this.cellLines*(this.cellLines-1)/2)+i]=cell;
...
var cell=this.allCells[$_dI((this.cellLines-1)*(this.cellLines-2)/2)+i];
...
var index=$_dI(this.cellLines*(this.cellLines-1)/2)+i;
...
this.cellLines--;
for(var i=0;i<this.cellLines;i++){
var cell=this.allCells[$_dI(this.cellLines*(this.cellLines-1)/2)+i];
...
$_M(c$,"initialize",
function(){
if(this.display!=null&&this.display.taskBar!=null&&(this.display.taskBar.orientation==1024||this.display.taskBar.orientation==128)){
return;
}{
...
this.display.taskBar.handle.appendChild(item);
return item;
}if(this.allItems.length+6-$_dI(this.cellLines*(this.cellLines+1)/2)>this.cellLines){
this.addTrayLine();
}var item=d$.createElement("DIV");
item.className="tray-item";
this.allItems[this.allItems.length]=item;
...
}if(this.itemCount==1&&this.display.taskBar!=null){
...
}var offset=(hideLogo?32:96);
if(this.display.trayCorner==null||this.display.trayCorner.orientation==131072){
item.style.left=(this.display.taskBar.clientWidth-order*32-offset)+"px";
}else{
item.style.left=(order*32+offset)+"px";
}if(this.display.taskBar.orientation==1024){
item.style.top=(this.display.taskBar.clientHeight-26)+"px";
}else{
item.style.top="12px";
}return;
}var index=-1;
...
if(order+6>=$_dI(i*(i+1)/2)){
index=order+6-$_dI(i*(i+1)/2);
currentLine=i;
break;
}}
var offset=0;
...
}if(this.allItems.length+6<=$_dI(this.cellLines*(this.cellLines+1)/2)){
this.removeTrayLine();
}break;
}}
if(this.display.trayCorner!=null){
this.display.trayCorner.setMinimized(false);
this.display.trayCorner.updateLastModified();
}if(this.itemCount==0&&this.display.taskBar!=null){
this.display.taskBar.updateItems();
}},"Element");
$_M(c$,"createItem",
function(item,index){
