...
}}var visibleCount=Math.min(count,$_dI(this.getDisplay().getBounds().height/this.lineHeight));
...
}this.setKeyBinding(88|$WT.MOD1,131199);
this.setKeyBinding(67|$WT.MOD1,17039369);
this.setKeyBinding(86|$WT.MOD1,16908297);
...
}}else if(this.selection.x==this.selection.y&&this.overwrite==true&&key!='\u0009'){
...
function(x,y,select){
var line=$_dI((y+this.verticalScrollOffset)/this.lineHeight);
...
var startLine=$_dI((y+this.verticalScrollOffset)/this.lineHeight);
var endY=y+height;
...
lineCount=$_dI((this.getClientArea().height-partialTopLineHeight)/this.lineHeight);
...
function(){
return $_dI(this.horizontalScrollOffset/this.getHorizontalIncrement());
});
...
var lineCount;
if(this.lineHeight!=0){
lineCount=$_dI(this.getClientArea().height/this.lineHeight);
}else{
lineCount=1;
}return lineCount;
});
...
var offsetInLine;
var lineText;
line=$_dI((this.getTopPixel()+point.y)/this.lineHeight);
...
var offsets=$_AI((styles.length-k)*2+2,0);
...
var partialTopIndex=$_dI(this.verticalScrollOffset/this.lineHeight);
...
}if(!ignore&&event.character.charCodeAt(0)>31&&event.character!='' || event.character == '\u000d' || event.character == '\u000a' || event.character == '\u0009'){
...
$_M(c$,"handlePaint",
function(event){
var startLine=Math.max(0,$_dI((event.y-this.topMargin+this.verticalScrollOffset)/this.lineHeight));
var paintYFromTopLine=(startLine-this.topIndex)*this.lineHeight;
var topLineOffset=this.topIndex*this.lineHeight-this.verticalScrollOffset;
...
var lineCount=this.content.getLineCount();
var oldBottomIndex=this.topIndex+$_dI(oldHeight/this.lineHeight);
...
if(height>0){
var lineCount=this.content.getLineCount();
var startLine=$_dI((this.getTopPixel()+y)/this.lineHeight);
...
this.setVerticalScrollOffset($_dI(this.verticalScrollOffset*this.lineHeight/oldLineHeight),true);
this.claimBottomFreeSpace();
}this.calculateContentWidth();
this.calculateScrollBars();
...
var verticalIncrement=this.getVerticalIncrement();
var horizontalIncrement=$_dI(clientAreaWidth/4);
var scrolled=false;
...
if($_dI((caretY+this.verticalScrollOffset)/this.getVerticalIncrement())-1!=caretLine){
...
if($_e(exception,NumberFormatException)){
}else{
throw exception;
}
}
...
this.lineWidth=$_AI(this.lineCount,0);
...
var lines=($_dI(this.b$["$wt.custom.StyledText"].autoScrollDistance/this.b$["$wt.custom.StyledText"].getLineHeight()))+1;
...
}this.pageSize=$_dI(this.clientArea.height/this.renderer.getLineHeight());
