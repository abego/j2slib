...
var gradientHeight=($_dI(this.gradientPercents[i]*rect.height/100))-pos;
this.fillGradientRectangle(0,pos,rect.width,gradientHeight,true,lastColorOld,lastColor);
pos+=gradientHeight;
}else{
var gradientWidth=($_dI(this.gradientPercents[i]*rect.width/100))-pos;
...
imageRect.width=16;
imageRect.height=16;
}this.drawImage(img,0,0,imageRect.width,imageRect.height,x,$_dI((rect.height-imageRect.height)/2),imageRect.width,imageRect.height);
x+=imageRect.width+5;
...
var lineY=Math.max(this.vIndent,rect.y+$_dI((rect.height-textHeight)/2));
for(var i=0;i<lines.length;i++){
var lineX=x;
if(lines.length>1){
if(this.align==16777216){
var lineWidth=this.textExtent(lines[i],$wt.custom.CLabel.DRAW_FLAGS).x;
lineX=x+Math.max(0,$_dI((extent.x-lineWidth)/2));
}if(this.align==131072){
var lineWidth=this.textExtent(lines[i],$wt.custom.CLabel.DRAW_FLAGS).x;
...
var red=r1+$_dI(i*(r2-r1)/gradientSteps);
var green=g1+$_dI(i*(g2-g1)/gradientSteps);
var blue=b1+$_dI(i*(b2-b1)/gradientSteps);
var color=new $wt.graphics.Color(null,red,green,blue);
var delta=-1;
if(vertical){
delta=i==gradientSteps?height-inc:$_dI((i+1)*height/(gradientSteps+1))-inc;
this.fillRectangle(x,y+inc,width,delta,color);
}else{
delta=i==gradientSteps?width-inc:$_dI((i+1)*width/(gradientSteps+1))-inc;
...
var crlf=(pos>0)&&(text.charAt(pos-1)=='\r');
