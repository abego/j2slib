...
var ratios=$_AI(cArray.length,0);
...
if($_O(children[i],$wt.widgets.Sash))continue;
...
}(data1).weight=$_dI(((b1.width<<16)+area.width-1)/area.width);
(data2).weight=$_dI(((b2.width<<16)+area.width-1)/area.width);
}else{
...
data2=new $wt.custom.SashFormData();
c2.setLayoutData(data2);
}(data1).weight=$_dI(((b1.height<<16)+area.height-1)/area.height);
(data2).weight=$_dI(((b2.height<<16)+area.height-1)/area.height);
...
}(data).weight=$_dI(((weights[i]<<16)+total-1)/total);
}
this.layout(false);
},"~A");
