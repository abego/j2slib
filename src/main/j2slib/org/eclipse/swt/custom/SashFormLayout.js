...
var ratios=$_AL(cArray.length,0);
var total=0;
for(var i=0;i<cArray.length;i++){
...
if(vertical){
height+=($_dI(total*maxValue/ratios[maxIndex]))+(cArray.length-1)*sashwidth;
}else{
width+=($_dI(total*maxValue/ratios[maxIndex]))+(cArray.length-1)*sashwidth;
...
var ratios=$_AL(controls.length,0);
var total=0;
for(var i=0;i<controls.length;i++){
...
var width=($_dI(ratios[0]*(area.width-sashes.length*sashwidth)/total));
var x=area.x;
...
x+=sashwidth;
width=($_dI(ratios[i]*(area.width-sashes.length*sashwidth)/total));
...
var height=($_dI(ratios[0]*(area.height-sashes.length*sashwidth)/total));
var y=area.y;
...
y+=sashwidth;
height=($_dI(ratios[i]*(area.height-sashes.length*sashwidth)/total));
