...
var outer=$_AI(shape.length,0);
for(var i=0;i<$_dI(shape.length/2);i++){
...
var from=lineRGB;
var to=outerRGB;
var red=from.red+$_dI(2*(to.red-from.red)/3);
var green=from.green+$_dI(2*(to.green-from.green)/3);
var blue=from.blue+$_dI(2*(to.blue-from.blue)/3);
var color=new $wt.graphics.Color(this.getDisplay(),red,green,blue);
gc.setForeground(color);
gc.drawPolyline(outer);
color.dispose();
}if(innerRGB!=null){
var inner=$_AI(shape.length,0);
var index=0;
var left=true;
var oldY=this.onBottom?0:this.getSize().y;
for(var i=0;i<$_dI(shape.length/2);i++){
...
var newPriority=$_AI(this.priority.length+1,0);
var next=0;
...
pos=$_dI(percents[percents.length-1]*height/100);
gc.setBackground(defaultBackground);
...
var gradientHeight=$_dI(percents[i]*height/100);
...
if(lastColor==null)lastColor=defaultBackground;
gc.setBackground(lastColor);
var gradientWidth=($_dI(percents[i]*width/100))-pos;
...
var fontHeight=$_dI(720/dpi.y);
var fd=this.getFont().getFontData()[0];
fd.setHeight(fontHeight);
var f=new $wt.graphics.Font(display,fd);
var fHeight=$_dI(f.getFontData()[0].getHeight()*dpi.y/72);
var indent=Math.max(2,$_dI((this.chevronRect.height-fHeight-4)/2));
...
var visible=$_AI(this.items.length,0);
var idx=0;
var current=-1;
for(var i=0;i<this.items.length;i++){
...
this.gradientPercents=$_AI(percents.length,0);
for(var i=0;i<percents.length;++i){
...
this.maxRect.y=this.onBottom?size.y-this.borderBottom-this.tabHeight+$_dI((this.tabHeight-18)/ 2) : this.borderTop + $_dI ((this.tabHeight - 18) /2);
...
this.minRect.y=this.onBottom?size.y-this.borderBottom-this.tabHeight+$_dI((this.tabHeight-18)/ 2) : this.borderTop + $_dI ((this.tabHeight - 18) /2);
...
this.chevronRect.y=this.onBottom?size.y-this.borderBottom-this.tabHeight+$_dI((this.tabHeight-this.chevronRect.height)/ 2) : this.borderTop + $_dI ((this.tabHeight - this.chevronRect.height) /2);
if(this.selectedIndex==-1){
...
item.closeRect.y=this.onBottom?size.y-this.borderBottom-this.tabHeight+$_dI((this.tabHeight-18)/ 2) : this.borderTop + $_dI ((this.tabHeight - 18) /2);
...
var minWidth=0;
var minWidths=$_AI(this.items.length,0);
for(var i=0;i<this.priority.length;i++){
...
var maxWidth=0;
var maxWidths=$_AI(this.items.length,0);
for(var i=0;i<this.items.length;i++){
...
var extra=$_dI((tabAreaWidth-minWidth)/this.items.length);
while(true){
var large=0;
var totalWidth=0;
for(var i=0;i<this.items.length;i++){
...
widths=$_AI(this.items.length,0);
for(var i=0;i<this.items.length;i++){
...
this.selectionGradientPercents=$_AI(percents.length,0);
for(var i=0;i<percents.length;++i){
...
var propTo=i;
var red=$_dI((to.red*propTo+from.red*propFrom)/denom);
var green=$_dI((to.green*propTo+from.green*propFrom)/denom);
var blue=$_dI((to.blue*propTo+from.blue*propFrom)/denom);
...
if(this.mru){
var newPriority=$_AI(this.priority.length,0);
...
var item=new $wt.widgets.MenuItem(this.$showMenu,0);
...
var width=0;
var widths=$_AI(this.items.length,0);
var gc=new $wt.graphics.GC(this);
...
this.curveIndent=$_dI(this.curveWidth/3);
}else{
var d=this.tabHeight-12;
