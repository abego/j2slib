...
var polygon=$_AI(2*count+2,0);
for(var i=0;i<=count;i++){
var t=i/count;
polygon[2*i]=$_dI(a0+a1*t+a2*t*t+a3*t*t*t);
polygon[2*i+1]=$_dI(b0+b1*t+b2*t*t+b3*t*t*t);
}
return polygon;
},"~N,~N,~N,~N,~N,~N,~N,~N,~N");
...
var line1=$_AI(this.curve.length+6,0);
var index=0;
var x=this.curveStart;
var y=0;
line1[index++]=x+1;
line1[index++]=size.y-1;
for(var i=0;i<$_dI(this.curve.length/2);i++){
...
var line2=$_AI(line1.length,0);
index=0;
for(var i=0;i<$_dI(line1.length/2);i++){
line2[index]=line1[index++]-1;
line2[index]=line1[index++];
}
var line3=$_AI(line1.length,0);
index=0;
for(var i=0;i<$_dI(line1.length/2);i++){
...
var red=from.red+$_dI(3*(to.red-from.red)/4);
var green=from.green+$_dI(3*(to.green-from.green)/4);
var blue=from.blue+$_dI(3*(to.blue-from.blue)/4);
