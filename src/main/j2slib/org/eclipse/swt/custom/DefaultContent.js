...
this.textStore=$_AC(0,'\0');
this.lines=$_AI(50,2,0);
});
$_K(c$,
function(){
this.setText("");
...
var newLines=$_AI(size+$wt.internal.Compatibility.pow2(this.expandExp),2,0);
...
if(count==size){
newLines=$_AI(size+$wt.internal.Compatibility.pow2(this.replaceExpandExp),2,0);
...
var indexedLines=$_AI(numLines,2,0);
var start=0;
var lineCnt=0;
var i;
this.replaceExpandExp=1;
...
var newLines=$_AI(lineCnt+1,2,0);
...
if(oldSize>0){
content=$_AC(this.textStore.length-oldSize,'\0');
...
return;
}content=$_AC(this.textStore.length+(newSize-oldSize),'\0');
...
return;
}var newLines=$_AI(size+Math.max(10,numLines),2,0);
System.arraycopy(this.lines,0,newLines,0,size);
this.lines=newLines;
},"~N");
$_M(c$,"error",
...
if(j+1<this.textStore.length)if(ch=='\u000d' && (this.textStore[j + 1] == '\u000a'))j++;
eol=true;
}}j++;
}
