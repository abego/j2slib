...
return $_dI(Math.cos(angle*$wt.internal.Compatibility.toRadians)*length);
},"~N,~N");
c$.sin=$_M(c$,"sin",
function(angle,length){
return $_dI(Math.sin(angle*$wt.internal.Compatibility.toRadians)*length);
},"~N,~N");
c$.ceil=$_M(c$,"ceil",
function(p,q){
return $_dI(Math.ceil(p/q));
},"~N,~N");
c$.floor=$_M(c$,"floor",
function(p,q){
return $_dI(Math.floor(p/q));
},"~N,~N");
c$.round=$_M(c$,"round",
function(p,q){
...
if($_e(ex,java.util.MissingResourceException)){
...
if($_e(ex2,java.util.MissingResourceException)){
}else{
throw ex2;
}
}
