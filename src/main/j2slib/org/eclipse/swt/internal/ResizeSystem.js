...
var ok=false;
var hdl=new $wt.internal.ResizeHandler(shell,status);
for(var i=0;i<$wt.internal.ResizeSystem.handlers.length;i++){
if($wt.internal.ResizeSystem.handlers[i]==null){
$wt.internal.ResizeSystem.handlers[i]=hdl;
ok=true;
break;
}}
if(!ok){
$wt.internal.ResizeSystem.handlers[$wt.internal.ResizeSystem.handlers.length]=hdl;
}if((status&268435456)!=0){
hdl.updateCornered();
}else if((status&16777216)!=0){
hdl.updateCentered();
