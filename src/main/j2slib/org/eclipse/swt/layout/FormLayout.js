...
if(bottom.offset<=0){
return $_dI(-top.offset*top.denominator/bottom.numerator);
}var divider=bottom.denominator-bottom.numerator;
return $_dI(bottom.denominator*bottom.offset/divider);
...
if(right.offset<=0){
return $_dI(-left.offset*left.denominator/left.numerator);
}var divider=right.denominator-right.numerator;
return $_dI(right.denominator*right.offset/divider);
...
if(flush==null)flush=$_Ab(children.length,false);
flush[i]=true;
}w=Math.max(x2,w);
if(move){
