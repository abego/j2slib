...
var expandCount=0;
var widths=$_AI(columnCount,0);
var minWidths=$_AI(columnCount,0);
var expandColumn=$_Ab(columnCount,false);
...
var delta=$_dI(w/spanExpandCount);
...
columnWidth=width==-1||expandCount==0?columnWidth:Math.max(minColumnWidth,$_dI(availableWidth/columnCount));
for(var i=0;i<columnCount;i++){
...
var c=expandCount;
var delta=$_dI((availableWidth-totalWidth)/c);
...
var delta2=$_dI(w/spanExpandCount);
...
delta=$_dI((availableWidth-totalWidth)/c);
...
expandCount=0;
var heights=$_AI(rowCount,0);
var minHeights=$_AI(rowCount,0);
var expandRow=$_Ab(rowCount,false);
for(var i=0;i<rowCount;i++){
for(var j=0;j<columnCount;j++){
var data=this.getData(grid,i,j,rowCount,columnCount,true);
if(data!=null){
...
var delta=$_dI(h/spanExpandCount);
...
var c=expandCount;
var delta=$_dI((availableHeight-totalHeight)/c);
...
var delta2=$_dI(h/spanExpandCount);
...
delta=$_dI((availableHeight-totalHeight)/c);
...
case 16777216:
case 2:
childX+=Math.max(0,$_dI((cellWidth-data.horizontalIndent-childWidth)/2));
break;
case 131072:
case 16777224:
case 3:
...
case 16777216:
case 2:
childY+=Math.max(0,$_dI((cellHeight-data.verticalIndent-childHeight)/2));
break;
case 1024:
case 16777224:
case 3:
