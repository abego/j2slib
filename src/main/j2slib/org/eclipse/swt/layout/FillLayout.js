...
w=Math.max(0,$_dI((wHint-(count-1)*this.spacing)/count));
}if(this.type==512&&hHint!=-1){
h=Math.max(0,$_dI((hHint-(count-1)*this.spacing)/count));
...
var cellWidth=$_dI(width/count);
for(var i=0;i<count;i++){
var child=children[i];
var childWidth=cellWidth;
if(i==0){
childWidth+=$_dI(extra/2);
}else{
if(i==count-1)childWidth+=$_dI((extra+1)/2);
...
var x=rect.x+this.marginWidth;
var cellHeight=$_dI(height/count);
var y=rect.y+this.marginHeight;
...
if(i==0){
childHeight+=$_dI(extra/2);
}else{
if(i==count-1)childHeight+=$_dI((extra+1)/2);
