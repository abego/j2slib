...
var margin=0;
if(!wrapped){
space=Math.max(0,$_dI((width-maxX)/(count+1)));
margin=Math.max(0,$_dI(((width-maxX)%(count+1))/2));
}else{
...
space=Math.max(0,$_dI((width-wrapX)/(wrapCount+1)));
margin=Math.max(0,$_dI(((width-wrapX)%(wrapCount+1))/2));
}for(var j=last;j<=i;j++){
...
var margin=0;
if(!wrapped){
space=Math.max(0,$_dI((height-maxY)/(count+1)));
margin=Math.max(0,$_dI(((height-maxY)%(count+1))/2));
}else{
if(this.fill||this.justify){
var last=0;
...
space=Math.max(0,$_dI((height-wrapY)/(wrapCount+1)));
margin=Math.max(0,$_dI(((height-wrapY)%(wrapCount+1))/2));
}for(var j=last;j<=i;j++){
