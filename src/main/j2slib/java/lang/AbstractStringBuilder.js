...
this.value=$_AC(capacity,'\0');
},"~N");
$_K(c$,
function(string){
this.count=string.length;
this.shared=false;
this.value=$_AC(this.count+16,'\0');
string.getChars(0,this.count,this.value,0);
},"~S");
$_M(c$,"enlargeBuffer",
($fz=function(min){
var twice=(this.value.length<<1)+2;
var newData=$_AC(min>twice?min:twice,'\0');
...
var newData=$_AC(this.value.length,'\0');
...
for(var i=0,end=this.count,mid=$_dI(this.count/2);i<mid;i++){
